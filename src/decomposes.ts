...
// 分解rgba颜色值
export const decomposeRgba = (color: string): IRgb => {
  const [r, g, b, a] = color
    .replace(/(^rgba?)|[()]/gi, '')
    .trim()
    .replace(/[,\s]+/g, ',')
...
// 分解十六进制颜色值
export const decomposeHexa = (color: string): IRgb => {
...
// 分解hsl颜色值并转rgb
export const decomposeHslaToRgba = (color: string): IRgb => {
...
// 分解hsv颜色值并转rgb
export const decomposeHsvaToRgba = (color: string): IRgb => {
